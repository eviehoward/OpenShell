...
Use `--from` to create a sandbox from the base image, another pre-built sandbox name, or a container image:
...
```shell
docker build -t my-image . # Docker gateway
podman build -t my-image . # Podman gateway
openshell sandbox create --from my-image
```
