...
Compute drivers other than MXC disable caller-supplied `template.driver_config`
by default. An operator can enable it without disabling resource approval:
...
driver table. Omitted admission settings use the labels above, except for MXC:
MXC allows caller driver config and disables resource label admission by
default. A supplied map
...
restriction remains. MXC host filesystem grants likewise cannot pass enabled
admission until a resolver exists. MXC permits its required command config by
default; VM upload workflows encoded in driver JSON require explicit
driver-config opt-in.
...
the `mxc` driver configuration when creating a sandbox. Unlike other drivers,
MXC defaults to `allow_driver_config = true` and
`resource_admission.enabled = false`: its required command uses driver config,
and its host filesystem grants have no trusted label resolver. The gateway
warns at startup that label admission is disabled. You can explicitly set
either field in this table; omitting `enabled` from a partial admission table
keeps it disabled. Setting `resource_admission.enabled = true` rejects
MXC sandbox creates until a trusted resolver is available.
