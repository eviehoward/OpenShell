...
Caller driver config is disabled by default except for MXC, whose required
command uses driver config. For drivers with resource admission enabled,
referenced external attachments must carry matching operator-controlled approval
labels. GPU device attachments are temporarily exempt. Host bind mounts,
supplemental image mounts, and MXC host filesystem grants lack a trusted label
resolver and are unavailable with admission enabled. MXC therefore disables
resource label admission by default and logs a warning at gateway startup.
Default MXC gateways do not use approval labels for host filesystem grants. An
operator can explicitly enable admission, which rejects MXC sandbox creates
until a trusted resolver is available.
